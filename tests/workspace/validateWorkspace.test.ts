...
describe("validateWorkspace", () => {
  ...
  it("warns on missing metadata and production-sensitive policy gaps", async () => {
    ...
    expect(result.issues.some((issue) => issue.message === "default_base missing")).toBe(true);
    expect(result.issues.some((issue) => issue.message === "remote missing")).toBe(true);
    expect(result.issues.some((issue) => issue.message.includes("docs_url"))).toBe(true);
    expect(
